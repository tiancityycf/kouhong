...
class Activity
{
	...
	{
		//前台测试链接：https://hz.zxmn2018.com/bxdj/api/v1_0_1/Activity/index.html&openid=oFGa94hlqh7ZTPk90Z0uihyEpGPc
		...
		//查询活动是否存在且申请时间是否在活动截止时间之前
			...
			if($history_groupId){
				...
				//（2）找到该用户结束的团队赛排名成绩
				$history_info = $groupModel->alias('g')->join('t_activity a','g.activity_id = a.id')->field('g.id,g.rank,g.group_steps,a.title,a.start_date,a.end_date')->where('g.id','in',$group_ids)->where('g.status',0)->order('a.id desc')->select();
