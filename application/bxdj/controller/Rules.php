...
class Rules extends BasicAdmin
{
    /**
     * 指定当前数据表
     * @var string
	...
	public function index()
    {
    	...
    	$this->title = '规则说明管理';
       	...
       	list($get, $db) = [$this->request->get(), new RulesModel()];
     ...
     protected function redisSave()
    {
        ...
        foreach ($arr as $k => $v) {
            if ( strpos($v['content'],'|')) {
                $arr[$k]['content'] = explode('|', $v['content']);
            }else{
                $arr[$k]['content'] = array($v['content']);
            }
        }

        if (Cache::set(config('rules_info'), $arr)) {
            return true;
        } else {
            return false;
        }
