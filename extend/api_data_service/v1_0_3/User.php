...
namespace api_data_service\v1_0_3;
...
use api_data_service\v1_0_7\Complain as ComplainService;
...
class User
{
    ...
    public function login($code, $from_type = 0)
    {
        ...
        if (isset($data['openid'])) {
            ...
            if ($user->userRecord->user_status == 2) {
                $user_status = 0;
            }
    ...
    public function update($data)
    {
        ...
        try {
            ...
            if ($zongheimingdan_config && $heimingdan_config && $user->userRecord->user_status != 0) {
                $arr['nickname'] = $user->nickname;
                $arr['user_status'] = $user->userRecord->user_status;
                $arr['timestamp'] = time();

                $sign = ComplainService::sign($arr);
                $arr['sign'] = $sign;
                ...
                if ($result['status'] === 200) {
                    $user_status = $result['data']['data']['user_status'];
                } else {
                    $user_status = $user->userRecord->user_status;
                }
                if ($user_status == 0) {
                    $user->userRecord->user_status = $user_status;
                } 
                $user->userRecord->save();
            } else {
                $user_status = $user->userRecord->user_status;
