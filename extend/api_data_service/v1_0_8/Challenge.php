...
class Challenge
{
	...
	public function end($data)
	{
        ...
        $insert_status = Db::name('record_log')->insert(['challenge_id' => $data['challenge_id']]);
		...
		try {
	        ...
	        if (isset($data['successed']) && $data['successed']) {
		        ...
		        $suipian = 0;
		        if ($user_status == 1) {
		        	$suipianService = new UserSuipianService();
					$suipian = $suipianService->suipian($data['user_id']);
		        }
	            

	            $result = ['status' => 1, 'suipian_type' => $user_status ? $suipian : 0, 'user_status' => $user_status];
	        }
