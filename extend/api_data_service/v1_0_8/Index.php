...
class Index
{
    ...
    {
        $cache = Cache::init();
        $heimingdan_key = config('heimingdan_redis_key');

        if (!$cache->has($heimingdan_key)) {
            $value = HeimingdanModel::column('nickname');
            $expire = 3600;
            Cache::set($heimingdan_key, $value, $expire);
        } else {
            $value = $cache->get($heimingdan_key);
        }

        $arr = array_rand($value, 10);
